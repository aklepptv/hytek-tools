...
def test_fix_cl2_middle_initial_command(tmp_path: Path, capsys) -> None:
    root = Path(__file__).resolve().parents[2]
    cl2_path = root / "sample" / "Meet Results-TOW vs SOUD 062326-23Jun2026-001.cl2"
    roster_path = root / "sample" / "roster.csv"
    ...
    assert output_path.is_file()
    assert "Fix CL2 Summary" in captured.out
    assert "Middle initials added: 261" in captured.out
    assert "Middle initials changed: 3" in captured.out
    assert "Records unchanged: 376" in captured.out
    assert captured.err == ""
